...
Кроме `PASS`/`FAIL` есть `WARN` (`warn(...)` в ассертах) — для известных, уже понятых расхождений:
печатаются каждый прогон, но не валят его, чтобы не маскировать регрессии. Сейчас таких нет.
Починил — переводи `warn(...)` обратно в `check(...)`.
...
`awp_reload` · `nade`. Добавить свой — вписать запись в
...
| `src/sim-core.js` | **детерминированное ядро движения** (Фаза 5, shared клиент↔сервер): чистая математика, состояние и BSP-халл передаются явно — без модульных глобалей. Dual-mode: классический браузерный скрипт + `module.exports` для `require` в Node. Камера/recoil/punch остались в `physics.js` | `simMakeHull`, `simMakeState`, `simPlayerMove(hull,st,cmd,dt,params)`, `simTraceMove`, `simCategorize`, `simPointContents`, `simGrenadeStep`/`simGrenadeThrow` (полёт гранаты — порт `SV_Physics_Bounce`+`BounceTouch`, общий с сервером) |
| `src/combat-core.js` | **математика хитрега пуль** (Фаза 5D): таблица урона/спада оружия, множители зон, stance-aware box-стек (голова/грудь/живот/ноги), ray-vs-box. Dual-mode — грузится И на сервере (авторитет), И в браузере (`enemyTryShoot` предсказывает попадания по сетевым тем же box-стеком → предсказание совпадает с сервером). Значения дублируют `WPNS`/`_HG_MULT` — держать в синхроне. Также **tagging** (замедление от попадания): `combatVelMod(wid,hg,ducked)` → `velMod` жертвы (0.5 малый флинч / 0.65 большой), которое демпфирует/восстанавливает `sim-core` на земле; сервер ставит в `worldProcessShot`, шлёт в снапшоте (`vm`), клиент применяет в reconciliation | `combatRayHitPlayer(o,d,pos,ducked)`, `combatDamage(wid,dist,hg,sil)`, `combatVelMod(wid,hg,ducked)`, `COMBAT_WEAPON_DMG`, `COMBAT_HG_MULT`, `COMBAT_PEN_MULT`, `COMBAT_LARGE_FLINCH` |
| `src/match-core.js` | **серверная логика матча** (Фаза 6): стейт-машина раунда (`warmup→buy→live→over`, таймеры, счёт, победа время/элиминация — 6A); **HP/броня/смерть/падение** (`matchApplyDamage`/`matchFallDamage`/`matchRevive` — 6B); **экономика** (`MATCH_BUY` каталог, `matchBuy` с валидацией, `matchKillReward`, награды раунда — 6C); **бомба C4** (`ms.bomb`, фьюз в `matchTick`, `matchBombPlant`/`matchBombDefuse`, `bombResult`, элиминация с поправкой на план — 6D; геометрия плант/дефьюз и радиус-урон — в `server.js`). Чистый dual-mode (только Node; клиент «ведомый» по `gstate`). Числа дублируют `rounds.js`/`game.js`/`WPNS` | `matchTick`, `matchApplyDamage`, `matchRevive`, `matchBuy`, `matchKillReward`, `matchBombPlant`/`matchBombDefuse`, `MATCH_*` |
...
- **Высота глаз — только через `playerEyeH()`** (`physics.js`): интерполяция `eyestand→eyeduck` **плюс
  +18 при `phyDucked`** (origin duck-халла на 18 ниже при тех же ногах). Все игровые лучи (выстрел, нож,
  гранаты, ракеты, подбор, декали) берут глаз отсюда; камера — сглаженную копию (`duckViewOfs`). Не
  писать `SV.eyestand + duckAmount*…` руками — забудешь +18, и присевший стреляет из колен.
